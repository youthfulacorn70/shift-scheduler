...
async def edit_rating(rating_id: int, data: dict):
    score = data.get("score")
    if score is None or score < 0 or score > 5:
        return {"error": "Score must be between 0 and 5."}
    update_rating(rating_id, score)
    return {"id": rating_id, "score": score}
...
async def remove_template(template_id: int, request: Request):
    manager_id = get_manager_id_from_request(request)
    deactivate_shift_template(template_id, manager_id)
    return {"message": "Recurring shift deactivated"}
